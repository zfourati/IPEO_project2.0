"""
This script processes Greenland imagery to combine the rgb bands, NDVI, NDWI and NDSI values,
apply data augmentation and apply them to a pretrained U-Net to predict landcover classification. 
"""
...
path_to_model = 'cnn_states/pretrainedUnet_features_DA'
os.makedirs(path_to_model, exist_ok=True)

path_to_plot = 'Plots/pretrainedUnet_features_DA'
...
class GreenlandData_Unet_features(Dataset):
    ...
    def __init__(self, split='train', transforms=False):
...
lib.ReshapeDataLoader(DataLoader(GreenlandData_Unet_features(split='train', transforms=True),
                                        batch_size=batch_size,
                                        shuffle='True',
                                        num_workers=1))

dl_train = lib.ReshapeDataLoader(DataLoader(GreenlandData_Unet_features(split='train', transforms=True), batch_size=batch_size, shuffle='True', num_workers=1))
...
dl_train_single = lib.ReshapeDataLoader(DataLoader(GreenlandData_Unet_features(split='train', transforms=True), batch_size=1, shuffle='False', num_workers=1))
